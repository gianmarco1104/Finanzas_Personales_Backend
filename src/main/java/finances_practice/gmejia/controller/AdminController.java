...
public class AdminController {
    ...
    public ResponseEntity<List<ListUsersResponse>> getUsersByStatus(@PathVariable Boolean status){
        return ResponseEntity.status(HttpStatus.OK).body(userService.getUsersByStatus(status));
    }

    @PatchMapping("/user/activate/{id}")
    public ResponseEntity<GeneralResponse> activate(@PathVariable Long id){
        return ResponseEntity.status(HttpStatus.OK).body(userService.userActivate(id));
    }

    @PatchMapping("/user/desactivate/{id}")
    public ResponseEntity<GeneralResponse> inactive(@PathVariable Long id){
