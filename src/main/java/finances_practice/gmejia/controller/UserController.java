...
public class UserController {
    ...
    public ResponseEntity<@NonNull GeneralResponse> register(@RequestBody @Valid RegisterRequest request){
        return ResponseEntity.status(HttpStatus.CREATED).body(userService.createUser(request));
    }

    @GetMapping("/profile")
    public ResponseEntity<@NonNull UserResponse> getProfile(){
        return ResponseEntity.status(HttpStatus.OK).body(userService.getMyProfile());
    }

    @PutMapping("/update")
    public ResponseEntity<@NonNull GeneralResponse> update(@RequestBody @Valid UpdateRequest request){
        return ResponseEntity.status(HttpStatus.OK).body(userService.updateUser(request));
    }

    @PatchMapping("/update/email")
    public ResponseEntity<@NonNull GeneralResponse> updateEmail(@RequestBody @Valid EmailUpdateRequest request){
        return ResponseEntity.status(HttpStatus.OK).body(userService.updateEmailUser(request));
    }

    @PatchMapping("/update/password")
    public ResponseEntity<@NonNull GeneralResponse> updatePassword(@RequestBody @Valid PasswordUpdateRequest request){
